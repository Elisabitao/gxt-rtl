...
public class DesktopApp implements EntryPoint {
	...
	public void onModuleLoad() {
        ...
        desktop =  new Desktop();
        desktop.getStartMenu().setToolWidth(125);
		...
		MenuItem skinsMenu=new MenuItem("Themes");
		skinsMenu.setIcon(IconHelper.createStyle("skins"));
		menu.addTool(skinsMenu);
		initSkinMenu(skinsMenu);
	}
	...
	private Theme last;
	protected void initSkinMenu(MenuItem menu)
	{
		...
		final SelectionListener<MenuEvent> menuListener = new SelectionListener<MenuEvent>() 
				{
			@Override
			public void componentSelected(MenuEvent me) {
				CheckMenuItem r=(CheckMenuItem)me.getItem();
				if(r.isChecked())
				{
					Theme c=r.getData("theme");
					if (c != last) {
						last = c;
						GXT.switchTheme(c);
					}
		...
		for(Theme t:themes)
		{
			CheckMenuItem r = new CheckMenuItem(t.getName());
			r.setData("theme",t);
			r.setGroup("skins");
			if(t.equals(Theme.BLUE) && themeId==null)
			{
				r.setChecked(true);
				last=t;
			}else if(themeId!=null&&t.getId().equals(themeId))
			{
				r.setChecked(true);  
				last=t;
			}
			r.addSelectionListener(menuListener);
			sub.add(r);  
